#!/usr/bin/env python3
"""
Run a Level 2 diagnostic linking Pacific COBE2 SST PCs 1-3 to Sierra ERA5-Land T2m anomalies.
...
3. Uses only PC1, PC2, and PC3 as predictors in a joint multivariate regression.
4. Subsets ERA5-Land monthly mean and climatology to the Sierra response domain.
5. Builds Sierra monthly T2m anomalies over the common overlap period.
6. Fits Y = A B + E and computes in-sample explained variance R^2(r).
"""
...
EXPERIMENT_NAME = "cobe2_pacific_sierra_t2m_level2_pc1to3"
MODEL_NAME = "PC1_to_PC3"
N_PREDICTORS = 3
PSCRATCH_OUTPUT_DIR = Path(
    os.environ.get(
        "COBE2_PACIFIC_SIERRA_T2M_LEVEL2_OUTPUT_DIR",
        "/pscratch/sd/h/hyvchen/Snow-Predication-at-Sierra/artifacts/cobe2_pacific_sierra_t2m_level2_pc1to3",
    )
)
HOME_OUTPUT_DIR = PROJECT_ROOT / "artifacts" / "cobe2_pacific_sierra_t2m_level2_pc1to3"
NETCDF_FILE = PSCRATCH_OUTPUT_DIR / "cobe2_pacific_sierra_t2m_level2_pc1to3.nc"
SUMMARY_CSV_FILE = HOME_OUTPUT_DIR / "cobe2_pacific_sierra_t2m_level2_pc1to3_summary.csv"
SUMMARY_JSON_FILE = HOME_OUTPUT_DIR / "cobe2_pacific_sierra_t2m_level2_pc1to3_summary.json"
FIGURE_FILE = HOME_OUTPUT_DIR / "cobe2_pacific_sierra_t2m_level2_pc1to3_maps.png"
...
def fit_multivariate_regression(
    ...
    predicted_mean_valid = np.mean(yhat_valid, axis=0)
    actual_mean_valid = np.mean(y_valid, axis=0)
    predicted_mean_map = np.full((y.shape[1], y.shape[2]), np.nan, dtype=np.float64)
    actual_mean_map = np.full((y.shape[1], y.shape[2]), np.nan, dtype=np.float64)
    predicted_mean_map.reshape(-1)[valid_columns] = predicted_mean_valid
    actual_mean_map.reshape(-1)[valid_columns] = actual_mean_valid

    r2_map = np.full((y.shape[1], y.shape[2]), np.nan, dtype=np.float64)
    r2_map.reshape(-1)[valid_columns] = r2_valid
    return coefficient_maps, r2_map, predicted_mean_map - actual_mean_map
...
def summarize_r2(
    ...
    return SummaryRow(
        ...
        pc1_std_raw=float(np.std(predictor_matrix[:, 0], ddof=1)),
        pc2_std_raw=float(np.std(predictor_matrix[:, 1], ddof=1)),
        pc3_std_raw=float(np.std(predictor_matrix[:, 2], ddof=1)),
    )
...
def save_netcdf(
    ...
    r2_map: np.ndarray,
    predicted_minus_actual_mean_map: np.ndarray,
    summary: SummaryRow,
    runtime,
) -> None:
    ds = xr.Dataset(
        data_vars={
            ...
            "sierra_predicted_minus_actual_mean": (
                ("sierra_latitude", "sierra_longitude"),
                predicted_minus_actual_mean_map.astype(np.float32),
            ),
    ...
    ds.to_netcdf(
        ...
        encoding={
            ...
            "sierra_predicted_minus_actual_mean": {"zlib": True, "complevel": 4, "shuffle": True, "_FillValue": np.float32(np.nan)},
...
def load_plot_inputs_from_netcdf() -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, Dict[str, float]]:
    with xr.open_dataset(NETCDF_FILE, engine="netcdf4", decode_times=True) as ds:
        ...
        predicted_minus_actual_mean_map = np.asarray(ds["sierra_predicted_minus_actual_mean"].values, dtype=np.float64)
        metrics = {
            "mean_r2": float(ds.attrs.get("sierra_area_weighted_mean_r2", np.nan))
            if "sierra_area_weighted_mean_r2" in ds.attrs
            else float(np.nan),
            "min_r2": float(ds.attrs.get("sierra_min_local_r2", np.nan))
            if "sierra_min_local_r2" in ds.attrs
            else float(np.nan),
            "max_r2": float(ds.attrs.get("sierra_max_local_r2", np.nan))
            if "sierra_max_local_r2" in ds.attrs
            else float(np.nan),
            "median_r2": float(ds.attrs.get("sierra_median_local_r2", np.nan))
            if "sierra_median_local_r2" in ds.attrs
            else float(np.nan),
        }
    return sierra_latitude, sierra_longitude, coefficient_maps, r2_map, predicted_minus_actual_mean_map, metrics


def diagnostic_text_box() -> str:
    return "\n".join(
        [
            r"$\hat{B}=(A^T A)^{-1}A^TY$",
            r"$A=[\mathrm{PC1},\mathrm{PC2},\mathrm{PC3}]$: predictor matrix.  "
            r"$Y$: Sierra ERA5-Land T2m anomaly matrix.",
            r"$[\hat{B}_1(r),\hat{B}_2(r),\hat{B}_3(r)]^T=(A^T A)^{-1}A^TY(:,r)$",
            r"$Y(:,r)$: full Sierra T2m anomaly time series at grid point $r$.",
            r"$\hat{B}_1(r), \hat{B}_2(r), \hat{B}_3(r)$: local T2m change from a one-unit increase in "
            r"PC1, PC2, or PC3, after accounting for the other two PCs.",
            r"If PCs are standardized, interpret coefficients as K per one-standard-deviation increase.",
            r"$\hat{Y}=A\hat{B}$: in-sample predicted Sierra T2m anomaly matrix.",
            r"$R^2(r)=1-\frac{\sum_t [Y(t,r)-\hat{Y}(t,r)]^2}{\sum_t [Y(t,r)]^2}$",
            r"$R^2(r)$: fraction of local Sierra T2m anomaly variance explained by PC1-PC3 together.",
            r"Example: $R^2(r)=0.12$ means 12\% of local variance is explained at grid point $r$.",
        ]
    )
...
def plot_maps(
    ...
    fig, axes = plt.subplots(2, 2, figsize=(14, 12.5), constrained_layout=False)
    fig.subplots_adjust(left=0.07, right=0.96, top=0.95, bottom=0.30, hspace=0.34, wspace=0.28)
    lon2d, lat2d = np.meshgrid(sierra_longitude, sierra_latitude)

    for mode_index, ax in enumerate(axes.flat[:3]):
        ...
        fig.colorbar(mesh, ax=ax, shrink=0.88).set_label(r"$\hat{B}_k(r)$ [K / raw PC unit]")
    ...
    fig.colorbar(r2_mesh, ax=r2_ax, shrink=0.88).set_label(r"$R^2(r)$")
...
def main() -> None:
    ...
    if plot_only:
        ...
        sierra_latitude, sierra_longitude, coefficient_maps, r2_map, predicted_minus_actual_mean_map, metrics = load_plot_inputs_from_netcdf()
        summary = SummaryRow(
            model_name=MODEL_NAME,
            n_pcs=N_PREDICTORS,
            overlap_start="",
        ...
        plot_maps(
            ...
            r2_map=r2_map,
            predicted_minus_actual_mean_map=predicted_minus_actual_mean_map,
            summary=summary,
        )
    ...
    try:
        ...
        print(
            "Computing Sierra ERA5-Land T2m Level 2 PC1-PC3 regression for overlap "
        ...
        coefficient_maps, r2_map, predicted_minus_actual_mean_map = fit_multivariate_regression(
