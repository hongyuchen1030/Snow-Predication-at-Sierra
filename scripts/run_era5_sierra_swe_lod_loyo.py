#!/usr/bin/env python3
"""
Run ERA5-only leave-one-water-year-out LOD for April 1 Sierra SWE.
"""
...
INSAMPLE_SUMMARY_FILE = Path("/pscratch/sd/h/hyvchen/Snow-Predication-at-Sierra/artifacts/era5_sierra_swe_lod_setup/lod_analysis/era5_sierra_swe_lod_summary.json")
PREDICTOR_FILE = Path("/pscratch/sd/h/hyvchen/Snow-Predication-at-Sierra/artifacts/era5_sierra_swe_lod_setup/predictors/era5_pacific_sst_monthly_anomaly_wy1985_2021_sep1984_mar2021.nc")
OUTPUT_ROOT = Path(
    os.environ.get(
        "ERA5_SIERRA_SWE_LOYO_LOD_ANALYSIS_ROOT",
...
def month_targets() -> tuple[list[np.datetime64], list[str]]:
    ...
    for water_year in WATER_YEARS:
        for _, year_offset, month in LAG_SPECS:
            ...
            month_keys.append(f"{year:04d}-{month:02d}-01")
    return times, month_keys
...
def validate_and_load_era5_predictor() -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    if not PREDICTOR_FILE.exists():
        raise FileNotFoundError(
            "Required ERA5 predictor file is missing: "
            f"{PREDICTOR_FILE}"
        )

    expected_times, expected_time_strings = month_targets()
    with xr.open_dataset(PREDICTOR_FILE, engine=NETCDF_ENGINE) as ds:
        attrs = ds.attrs
        required_vars = {"sst_monthly_mean", "sst_monthly_anomaly", "valid_ocean_mask"}
        missing_vars = sorted(required_vars.difference(ds.data_vars))
        if missing_vars:
            raise ValueError(f"ERA5 predictor file is missing required variables: {missing_vars}")

        if attrs.get("source_dataset") != "ERA5":
            raise ValueError(f"ERA5 predictor source_dataset mismatch: {attrs.get('source_dataset')!r}")
        if attrs.get("variable_name") != ERA5_VARIABLE:
            raise ValueError(f"ERA5 predictor variable_name mismatch: {attrs.get('variable_name')!r}")
        if int(attrs.get("water_year_start")) != WATER_YEAR_START or int(attrs.get("water_year_end")) != WATER_YEAR_END:
            raise ValueError(
                "ERA5 predictor water-year coverage mismatch: "
                f"{attrs.get('water_year_start')}..{attrs.get('water_year_end')}"
            )
        if float(attrs.get("domain_lat_min")) != PACIFIC_LAT_MIN or float(attrs.get("domain_lat_max")) != PACIFIC_LAT_MAX:
            raise ValueError("ERA5 predictor latitude domain mismatch.")
        if float(attrs.get("domain_lon_min_360")) != PACIFIC_LON_MIN or float(attrs.get("domain_lon_max_360")) != PACIFIC_LON_MAX:
            raise ValueError("ERA5 predictor longitude domain mismatch.")

        lag_names = [str(v) for v in ds["lag_month"].values.tolist()]
        if lag_names != EXPECTED_LAG_NAMES:
            raise ValueError(f"ERA5 predictor lag-month list mismatch: {lag_names}")
        ...
        valid_ocean_mask = np.asarray(ds["valid_ocean_mask"].values, dtype=bool)
        latitude = np.asarray(ds["latitude"].values, dtype=np.float32)
        longitude = np.asarray(ds["longitude"].values, dtype=np.float32)
        time_values = np.asarray(ds["time"].values).astype("datetime64[D]")

    if monthly_mean.shape[0] != EXPECTED_MONTH_COUNT:
        raise ValueError(f"ERA5 predictor monthly_mean time length mismatch: {monthly_mean.shape[0]}")
    if monthly_anom.shape[0] != len(WATER_YEARS) or monthly_anom.shape[1] != len(LAG_SPECS):
        raise ValueError(f"ERA5 predictor anomaly shape mismatch: {monthly_anom.shape}")
    if str(time_values[0]) != expected_time_strings[0] or str(time_values[-1]) != expected_time_strings[-1]:
        raise ValueError(
            "ERA5 predictor time coverage mismatch: "
            f"{time_values[0]} .. {time_values[-1]}"
        )
    expected_times_np = np.asarray(expected_times, dtype="datetime64[D]")
    if not np.array_equal(time_values, expected_times_np):
        raise ValueError("ERA5 predictor time coordinate does not match required Sep 1984 through Mar 2021 alignment.")

    reshaped_monthly = monthly_mean.reshape(len(WATER_YEARS), len(LAG_SPECS), latitude.size, longitude.size)
    if reshaped_monthly.shape != monthly_anom.shape:
        raise ValueError(
            "ERA5 predictor monthly_mean / anomaly shape mismatch after reshape: "
            f"{reshaped_monthly.shape} vs {monthly_anom.shape}"
        )
    return reshaped_monthly, valid_ocean_mask, latitude, longitude
...
def build_fold_predictors(
    ...
    valid = shared_valid_ocean[None, :, :] & np.all(np.isfinite(train_anom), axis=0) & np.isfinite(std) & (std > 0.0)
...
def run_lod_fold(
    ...
    lag_names = np.asarray(EXPECTED_LAG_NAMES, dtype="U3")
...
def plot_scatter(path: Path, observed: np.ndarray, predicted: np.ndarray, r2_loyo: float, corr: float) -> None:
    ...
    ax.set_title(f"ERA5-only LOYO LOD: observed vs predicted\nR2={r2_loyo:.3f}  corr={corr:.3f}")
...
def plot_timeseries(path: Path, water_years: np.ndarray, observed: np.ndarray, predicted: np.ndarray) -> None:
    ...
    ax.set_title("ERA5-only LOYO LOD by water year")
...
def skill_label(r2_loyo: float) -> str:
    if r2_loyo > 0.0:
        return "positive skill"
    if r2_loyo < 0.0:
        return "worse than mean prediction"
    return "near-zero skill"


def mode_rank_stability_label(region_summary: dict[str, object], threshold_fraction: float) -> str:
    fraction = float(region_summary.get("top_region_fraction", float("nan")))
    if np.isfinite(fraction) and fraction >= threshold_fraction:
        return "rank-stable"
    return "not strongly rank-stable"


def pooled_late_mode_behavior(fold_rows: list[dict[str, object]]) -> str:
    pooled_rows = [row for row in fold_rows if row.get("selected") and 3 <= int(row["mode_number"]) <= 6]
    counts: dict[tuple[float, float], int] = {}
    for row in pooled_rows:
        key = (float(row["latitude"]), float(row["longitude_0_360"]))
        counts[key] = counts.get(key, 0) + 1
    if not counts:
        return "no pooled later-mode selections"
    if max(counts.values()) >= 5:
        return "recurring regions"
    return "scattered later-mode behavior"
...
def write_summary_markdown(
    ...
    pooled_note_path: Path,
    mode1_label: str,
    mode2_label: str,
    pooled_behavior: str,
) -> None:
    lines = [
        "# ERA5-only LOYO LOD Summary",
        "",
        "- This run is **ERA5-only**.",
        ...
        f"- LOYO observed-vs-predicted correlation: `{skill_summary['metrics']['correlation']:.4f}`",
        f"- Held-out prediction verdict: **{skill_summary['interpretation']['skill_label']}**",
        f"- Mean selected mode count per fold: `{skill_summary['mode_selection']['mean_selected_modes_per_fold']:.3f}`",
        "",
        "## Mode Stability",
        "",
        f"- Mode 1 is **{mode1_label}**: top region `{repeated_regions[0]['top_region']}` in `{repeated_regions[0]['top_region_count']}/{repeated_regions[0]['selected_fold_count']}` selected folds.",
        f"- Mode 2 is **{mode2_label}**: top region `{repeated_regions[1]['top_region']}` in `{repeated_regions[1]['top_region_count']}/{repeated_regions[1]['selected_fold_count']}` selected folds.",
        f"- Pooled modes 3--6 are most consistent with **{pooled_behavior}**.",
        f"- Pooled modes 3--6 interpretation note: `{pooled_note_path.name}`",
        f"- Six-panel exact-rank selection map: `{selection_map_path.name}`",
        "",
        "## Outputs",
        "",
        "- `era5_sierra_swe_lod_loyo_predictions.csv`",
        "- `era5_sierra_swe_lod_loyo_fold_modes.csv`",
        "- `era5_sierra_swe_lod_loyo_skill_summary.json`",
        "- `era5_sierra_swe_lod_loyo_mode_stability.csv`",
        "- `era5_sierra_swe_lod_loyo_observed_vs_predicted.png`",
        "- `era5_sierra_swe_lod_loyo_timeseries.png`",
        "- `era5_sierra_swe_lod_loyo_summary.md`",
        "- `era5_lod_loyo_mode1_mode2_latepooled_selection_frequency.png`",
        "- `era5_lod_loyo_mode1_mode2_latepooled_selection_frequency.pdf`",
        "- `era5_lod_loyo_mode1_mode2_latepooled_selection_frequency.csv`",
        "- `era5_lod_loyo_mode_stability_interpretation.md`",
    ]
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    print("validating and loading processed ERA5 monthly-mean predictor...", flush=True)
    sst_values, shared_valid_ocean, latitude, longitude = validate_and_load_era5_predictor()
    print(f"loaded ERA5 monthly means shape={sst_values.shape}", flush=True)
    ...
    print(f"Starting ERA5-only LOYO LOD across {WATER_YEARS.size} folds", flush=True)
    for fold_index, held_out_wy in enumerate(WATER_YEARS):
        ...
        X_train, x_test, lag_idx, spatial_idx, valid_counts_by_lag = build_fold_predictors(train_sst, test_sst, shared_valid_ocean)
    ...
    predictions_csv = OUTPUT_ROOT / "era5_sierra_swe_lod_loyo_predictions.csv"
    fold_modes_csv = OUTPUT_ROOT / "era5_sierra_swe_lod_loyo_fold_modes.csv"
    skill_summary_json = OUTPUT_ROOT / "era5_sierra_swe_lod_loyo_skill_summary.json"
    mode_stability_csv = OUTPUT_ROOT / "era5_sierra_swe_lod_loyo_mode_stability.csv"
    scatter_png = OUTPUT_ROOT / "era5_sierra_swe_lod_loyo_observed_vs_predicted.png"
    timeseries_png = OUTPUT_ROOT / "era5_sierra_swe_lod_loyo_timeseries.png"
    summary_md = OUTPUT_ROOT / "era5_sierra_swe_lod_loyo_summary.md"
    selection_map_png = OUTPUT_ROOT / "era5_sierra_swe_lod_loyo_selection_frequency_map.png"
    pooled_note_path = OUTPUT_ROOT / "era5_lod_loyo_mode_stability_interpretation.md"
    ...
    mode1_label = mode_rank_stability_label(repeated_regions[0], 0.5)
    mode2_label = mode_rank_stability_label(repeated_regions[1], 0.4)
    pooled_behavior = pooled_late_mode_behavior(fold_mode_rows)
    ...
    skill_summary = {
        ...
        "predictor_definition": "ERA5 monthly SST anomalies Sep-Mar over Pacific box lat -10..60 lon 120..280 in 0..360",
        "predictor_source": str(PREDICTOR_FILE),
        ...
        "interpretation": {
            ...
            "skill_label": skill_label(r2_loyo),
            "mode1_rank_stability": mode1_label,
            "mode2_rank_stability": mode2_label,
            "pooled_modes3to6_behavior": pooled_behavior,
    ...
    write_summary_markdown(summary_md, skill_summary, repeated_regions, selection_map_png, pooled_note_path, mode1_label, mode2_label, pooled_behavior)
