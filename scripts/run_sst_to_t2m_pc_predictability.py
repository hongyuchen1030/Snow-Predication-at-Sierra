#!/usr/bin/env python3
"""
Predict March mean overland T2M PCs from February SST PCs.

This script reuses:
- COBE2 no-centering SVD EOF basis projected model SST PCs
- Precomputed WUS-D3 d01 March t2 anomaly products

For each historical model, it:
1. Loads February SST_PC1 and SST_PC2.
2. Loads March mean overland t2 anomaly fields on WUS-D3 d01.
3. Computes land-only March T2M EOFs / PCs from the anomaly fields.
4. Runs leave-one-water-year-out ridge regression to predict T2M_PC1/2.
5. Saves metrics, timeseries, EOF/PC artifacts, plots, and a markdown report.
"""
...
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "artifacts" / "sst_to_t2m_pc_predictability"
OUTPUT_DIR = Path(os.environ.get("SST_TO_T2M_PC_OUTPUT_DIR", str(DEFAULT_OUTPUT_DIR)))
...
def ensure_output_dirs() -> None:
    for path in (OUTPUT_DIR, PLOTS_DIR, EOFS_DIR, PCS_DIR):
...
def compute_t2m_pca(anomaly_path: Path, wrf_land_mask: np.ndarray) -> T2mPcaResult:
    ...
    print(
        f"computed T2M PCA from {anomaly_path.name}: years={len(years)} valid_land_cells={valid_count} "
        f"matrix_shape={matrix.shape} evr_first2={explained_variance_ratio[:2].tolist()}",
        flush=True,
    )
...
def build_samples(
    ...
    print(
        f"built samples for {model_name}: overlapping_years={len(samples)} "
        f"range={samples[0]['year']}-{samples[-1]['year']}",
        flush=True,
    )
    return samples
...
def run_leave_one_year_out(samples: Sequence[Dict[str, object]]) -> List[FoldPrediction]:
    ...
    for target_index in range(N_TARGET_COMPONENTS):
        ...
        for test_year in years:
            ...
            print(
                f"LOYO model={test_samples[0]['model']} target={target_pc_name} held_out_year={test_year}",
                flush=True,
            )
    ...
    dataset = xr.Dataset(
        data_vars={
            "t2m_eof": (
                ...
                eof_cube,
                {
                    "description": "Land-only March T2M EOFs from SVD of anomaly fields",
...
def plot_model_timeseries(model_name: str, rows: Sequence[FoldPrediction], target_pc: str, path: Path) -> None:
    ...
    ax.set_title(f"{model_name} February SST -> March {target_pc}")
...
def plot_pooled_timeseries(rows: Sequence[FoldPrediction], target_pc: str, path: Path) -> None:
    ...
    fig.suptitle(f"February SST -> March {target_pc} temporal evolution by model")
    fig.savefig(path, dpi=150)
    plt.close(fig)
...
def plot_model_eofs(model_name: str, result: T2mPcaResult, path: Path) -> None:
    ...
    fig.suptitle(f"{model_name} March T2M EOFs")
    fig.savefig(path, dpi=150)
    plt.close(fig)


def plot_explained_variance(model_name: str, result: T2mPcaResult, path: Path) -> None:
    count = min(10, result.explained_variance_ratio.shape[0])
    components = np.arange(1, count + 1, dtype=np.int32)
    values = result.explained_variance_ratio[:count]

    fig, ax = plt.subplots(figsize=(8, 4))
    ax.bar(components, values, color="#4C78A8")
    ax.plot(components, np.cumsum(values), marker="o", color="#F58518", label="cumulative")
    ax.set_xlabel("Component")
    ax.set_ylabel("Explained variance ratio")
    ax.set_title(f"{model_name} March T2M explained variance")
...
def plot_correlation_summary(summary_rows: Sequence[Dict[str, object]], path: Path) -> None:
    ...
    pc1 = [float(next(row["correlation"] for row in selected if row["model"] == model_name and row["target_pc"] == "T2M_PC1")) for model_name in model_names]
    pc2 = [float(next(row["correlation"] for row in selected if row["model"] == model_name and row["target_pc"] == "T2M_PC2")) for model_name in model_names]
    ...
    ax.set_title("February SST -> March T2M PC correlation by model")
    ax.grid(True, alpha=0.25, axis="y")
    ax.legend()
    fig.tight_layout()
    fig.savefig(path, dpi=150)
...
def plot_r2_summary(summary_rows: Sequence[Dict[str, object]], path: Path) -> None:
    selected = [row for row in summary_rows if row["model"] != "ALL"]
    model_names = sorted({str(row["model"]) for row in selected})
    pc1 = [float(next(row["r2"] for row in selected if row["model"] == model_name and row["target_pc"] == "T2M_PC1")) for model_name in model_names]
    pc2 = [float(next(row["r2"] for row in selected if row["model"] == model_name and row["target_pc"] == "T2M_PC2")) for model_name in model_names]
    ...
    ax.set_xticklabels(model_names, rotation=20, ha="right")
    ax.set_ylabel("R2")
    ax.set_title("February SST -> March T2M PC R2 by model")
    ax.grid(True, alpha=0.25, axis="y")
    ax.legend()
    fig.tight_layout()
    fig.savefig(path, dpi=150)
...
def write_report(
    ...
    predictive_signal = "some" if pooled_rows["T2M_PC1"]["correlation"] > 0.0 or pooled_rows["T2M_PC2"]["correlation"] > 0.0 else "little"

    lines = [
        "# SST -> March T2M PC Predictability",
        "",
        "## Runtime",
        "",
        ...
        "## Setup",
        "",
        "- SST inputs reused existing COBE2 no-centering SVD projected model SST PCs from `artifacts/sst_pca/model_sst_pcs/`.",
        "- Predictors used only February `SST_PC1` and `SST_PC2`, with the February month-end immediately preceding the target March.",
        "- T2M targets reused saved WUS-D3 d01 March `t2` anomaly fields from `artifacts/t2m_anomaly_validation/wus_t2m_march_anomalies/`.",
        "- T2M EOFs/PCs were computed over land cells only (`LANDMASK == 1`) using SVD on the March anomaly matrix for each historical model.",
        "- Validation used leave-one-water-year-out ridge regression with one held-out year per fold.",
        f"- Ridge alpha: `{RIDGE_ALPHA}`.",
        "- The primary diagnostic products are yearly predicted-vs-true PC trajectory plots, so the main scientific readout is whether the reconstructed PC curves follow interannual variability over time.",
        "",
        "## Main Findings",
        "",
        f"- February SST PCs contain {predictive_signal} predictive information for March low-order T2M variability. "
        f"Pooled correlation is `{pooled_rows['T2M_PC1']['correlation']:.3f}` for `T2M_PC1` and "
        f"`{pooled_rows['T2M_PC2']['correlation']:.3f}` for `T2M_PC2`.",
        f"- `T2M_{better_pc}` is more predictable than the other retained temperature mode in this experiment.",
        f"- Across models, mean correlation is `{mean_pc1_corr:.3f}` for `T2M_PC1` and `{mean_pc2_corr:.3f}` for `T2M_PC2`; "
        f"mean R2 is `{mean_pc1_r2:.3f}` and `{mean_pc2_r2:.3f}` respectively.",
        "- This experiment supports the idea that low-order atmospheric temperature variability is easier to capture than full raw anomaly maps, "
        "but that conclusion is indirect here because no raw-map prediction baseline was run in the same script.",
        "",
        ...
        "- Year-by-year predicted and true PC values are tabulated in `prediction_timeseries.csv` for direct inspection or downstream plotting.",
        "",
        "## Model Coverage",
        "",
    ]
    ...
    for model_name in models:
        lines.append(
            f"- `{model_name}`: {len(explained_variance[model_name])} retained SVD modes available, "
            f"first two explained-variance ratios = "
...
def main() -> None:
    ...
    print(f"historical models: {model_names}", flush=True)

    for model_name in model_names:
        sst_path = SST_PC_DIR / f"{model_name}_sst_pcs.csv"
        anomaly_path = T2M_ANOMALY_DIR / f"{model_name}_historical_march_t2_anomalies.nc"
        if not sst_path.exists():
            raise FileNotFoundError(f"Missing SST PC file for {model_name}: {sst_path}")
        if not anomaly_path.exists():
            raise FileNotFoundError(f"Missing March t2 anomaly file for {model_name}: {anomaly_path}")
        ...
        plot_explained_variance(
            model_name,
            t2m_pca,
            PLOTS_DIR / f"{model_name}_march_t2m_explained_variance.png",
        )
    ...
    write_report(REPORT_FILE, runtime, model_names, explained_variance, summary_rows)
