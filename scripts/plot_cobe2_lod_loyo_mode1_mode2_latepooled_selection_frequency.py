#!/usr/bin/env python3
"""
Create a COBE2 LOYO LOD mode-stability plot with mode 1, mode 2, and pooled modes 3-6.
"""
...
INPUT_CSV = Path(
    os.environ.get(
        "COBE2_LOYO_FOLD_MODES_CSV",
        "/pscratch/sd/h/hyvchen/Snow-Predication-at-Sierra/artifacts/cobe2_sierra_swe_lod_setup/loyo_lod_analysis/cobe2_sierra_swe_lod_loyo_fold_modes.csv",
    )
).expanduser()
OUTPUT_ROOT = Path(
    os.environ.get(
        "COBE2_LOYO_MODE_STABILITY_OUTPUT_ROOT",
...
def panel_scatter(ax: plt.Axes, counts: dict[tuple[float, float], int], title: str, vmax: int) -> None:
...
def write_note(path: Path, pooled_total: int) -> None:
    lines = [
        "# COBE2 LOYO Mode-Stability Interpretation",
        "",
        "- Mode 1 and mode 2 are shown separately because they appear more rank-stable across LOYO folds.",
...
def main() -> None:
    ...
    summary_csv = OUTPUT_ROOT / "cobe2_lod_loyo_mode1_mode2_latepooled_selection_frequency.csv"
    plot_png = OUTPUT_ROOT / "cobe2_lod_loyo_mode1_mode2_latepooled_selection_frequency.png"
    plot_pdf = OUTPUT_ROOT / "cobe2_lod_loyo_mode1_mode2_latepooled_selection_frequency.pdf"
    note_md = OUTPUT_ROOT / "cobe2_lod_loyo_mode_stability_interpretation.md"
    ...
    write_note(note_md, len(pooled_late_rows))
