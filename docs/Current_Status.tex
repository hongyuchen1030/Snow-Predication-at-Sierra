...
\subsection{Latest Session: Known-Climate-Mode SWE Baseline}
...
prediction project: a known-climate-mode baseline. Instead of starting immediately from the full
Pacific SST grid, we first build a compact predictor set from ocean-side climate-variability modes
that are physically relevant to western U.S. hydroclimate and may help explain Sierra Nevada SWE
predictability. The initial ocean-mode baseline is restricted to three predictor groups: COBE2
broad Pacific SST EOF-PC modes, the Ni\~no 3.4 SST index, and AMV/AMO North Atlantic SST EOF-PC
modes. The purpose of this baseline is to test how much April 1 Sierra SWE predictability can be
explained by established oceanic climate-mode predictors before moving to direct full-field SST
discovery. MJO is intentionally excluded from this ocean-mode baseline. MJO is an
atmospheric/subseasonal mode and should be handled later using appropriate phase/amplitude indices
rather than monthly SST EOFs.
...
\centering
\small
\begin{tabular}{>{\raggedright\arraybackslash}p{0.12\linewidth} >{\raggedright\arraybackslash}p{0.08\linewidth} >{\raggedright\arraybackslash}p{0.17\linewidth} >{\raggedright\arraybackslash}p{0.18\linewidth} >{\raggedright\arraybackslash}p{0.21\linewidth} >{\raggedright\arraybackslash}p{0.16\linewidth}}
\toprule
Mode / index & Variable & Domain & Method & Expected predictor columns & Storage location / status \\
\midrule
COBE2 Pacific SST modes & SST & $10^\circ\mathrm{S}$--$60^\circ\mathrm{N}$, $120^\circ\mathrm{E}$--$280^\circ\mathrm{E}$ in $0..360$, equivalently $120^\circ\mathrm{E}$--$80^\circ\mathrm{W}$ & EOF1--EOF6 / PC1--PC6 from the existing COBE2 broad Pacific SST EOF workflow & Sep--Mar Pacific SST PC1--PC6 values. These modes may contain basin-wide warming, ENSO-like variability, and PDO-like variability, but those interpretations should only be assigned after inspecting the EOF maps and PC correlations. & Reuse existing generated COBE2 Pacific EOF/PC artifacts. Do not regenerate unless missing or unusable: \path{artifacts/sst_pca/cobe2_global_monthly_climatology_anomaly/cobe2_global_monthly_clim_sst_eofs.nc}; \path{artifacts/cobe2_pacific_sierra_t2m_level1_diagnostic/cobe2_pacific_sierra_cobe2_eofs_modes1to6.png}; \path{/pscratch/sd/h/hyvchen/Snow-Predication-at-Sierra/artifacts/cobe2_pacific_sierra_t2m_level2_pc1to6/cobe2_pacific_sierra_t2m_level2_pc1to6.nc}; \path{artifacts/cobe2_pacific_sierra_t2m_level2_pc1to6/run.log}. \\
Ni\~no 3.4 & SST & $5^\circ\mathrm{S}$--$5^\circ\mathrm{N}$, $190^\circ\mathrm{E}$--$240^\circ\mathrm{E}$ in $0..360$, equivalently $170^\circ\mathrm{W}$--$120^\circ\mathrm{W}$ & Area-weighted domain average of monthly SST anomalies & \texttt{Nino34\_Sep}, \texttt{Nino34\_Oct}, \texttt{Nino34\_Nov}, \texttt{Nino34\_Dec}, \texttt{Nino34\_Jan}, \texttt{Nino34\_Feb}, \texttt{Nino34\_Mar} & Already generated: \path{artifacts/swe_climate_mode_baseline/nino34/nino34_monthly_wy1985_2021_sep_mar.csv}; \path{artifacts/swe_climate_mode_baseline/nino34/nino34_monthly_wy1985_2021_sep_mar.nc}; \path{artifacts/swe_climate_mode_baseline/nino34/nino34_monthly_wy1985_2021_sep_mar_summary.json}. \\
AMV / AMO & SST & $0^\circ$--$70^\circ\mathrm{N}$, $280^\circ\mathrm{E}$--$360^\circ\mathrm{E}$ in $0..360$, equivalently $80^\circ\mathrm{W}$--$0^\circ$ & Separate EOF1--EOF6 / PC1--PC6 analysis over the North Atlantic SST anomaly field; this does not reuse the Pacific EOF basis & Sep--Mar AMV/AMO PC1--PC6 values & Generated: \path{artifacts/swe_climate_mode_baseline/amv_amo/amv_amo_cobe2_north_atlantic_eofs_pc1to6.nc}; \path{artifacts/swe_climate_mode_baseline/amv_amo/amv_amo_cobe2_north_atlantic_pc1to6_wy1985_2021_sep_mar.csv}; \path{artifacts/swe_climate_mode_baseline/amv_amo/amv_amo_cobe2_north_atlantic_pc1to6_summary.json}; \path{artifacts/swe_climate_mode_baseline/amv_amo/amv_amo_cobe2_north_atlantic_eofs_modes1to6.png}; \path{artifacts/swe_climate_mode_baseline/amv_amo/run.log}. \\
\bottomrule
\end{tabular}
\caption{Ocean-side known-climate-mode baseline predictors for the Sierra SWE project.}
