...
\begin{figure}[h]
\centering
\includegraphics[width=0.49\linewidth]{../figures/cobe2_pacific_sierra_cobe2_eofs_modes1to6.pdf}
\includegraphics[width=0.49\linewidth]{../figures/cobe2_pacific_sierra_t2m_level1_maps_modes1to6.pdf}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.82\linewidth]{../figures/cobe2_pacific_sierra_t2m_level2_pc1to6_maps.pdf}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.88\linewidth]{../figures/cobe2_pacific_sierra_t2m_level2_pc1to6_monthly_r2_maps.pdf}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.95\linewidth]{../figures/cobe2_eof1_and_wusd3_d01_pacific_coverage.pdf}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.88\linewidth]{../figures/cobe2_sharedregion_sierra_t2m_level2_pc1to6_monthly_r2_maps.pdf}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.88\linewidth]{../figures/wusd3_sharedregion_pc_t2m_level2_pc1to6_monthly_r2_maps.pdf}
...
\paragraph{COBE2 vs ERA5 LOD Modes 1 to 6 Correlation Heat Maps}
Figure~\ref{fig:cobe2_vs_era5_lod_modes_1to6_heatmaps} compares the COBE2 and ERA5 residual-correlation heat maps for the retained LOD modes.
...
Mode 2 & Mode 1 \\
Mode 3 & Mode 3 \\
Modes 4/5 & roughly Modes 4/5 \\
Mode 6 & Mode 6 \\
\hline
\end{tabular}
\caption{Mode matching used when comparing the COBE2 and ERA5 heat maps in Figure~\ref{fig:cobe2_vs_era5_lod_modes_1to6_heatmaps}.}
\label{tab:cobe2_era5_mode_matching}
\end{table}

\begin{figure}[h]
\centering
\includegraphics[width=0.96\linewidth]{../figures/cobe2_vs_era5_lod_modes_1to6_heatmaps.pdf}
\caption{
COBE2-versus-ERA5 SST LOD residual-correlation heat maps. LOD first searches all SST predictor
columns and selects one mode column. That selected column determines which month is shown in each
panel. The heat-map colors then show the correlations between the current SWE residual and all
grid-cell SST columns within that selected month.
}
\label{fig:cobe2_vs_era5_lod_modes_1to6_heatmaps}
\end{figure}
...
The target is April 1 SWE over the Sierra Nevada, possibly separated into northern, central, and
...
The Sierra SWE dataset visualization is provided in Fig.~\ref{fig:swe_dem_four_panel_diagnostic}.


To define regional SWE targets, we divide the Sierra SWE grid into northern, central, and southern
regions. The SWE regions are defined using the CDEC/DWR basin-based
snowpack grouping:
\[
\begin{aligned}
\text{North Sierra} &: \text{Trinity through Feather and Truckee},\\
\text{Central Sierra} &: \text{Yuba and Tahoe through Merced and Walker},\\
\text{South Sierra} &: \text{San Joaquin and Mono through Kern}.
\end{aligned}
\]

The resulting Sierra SWE regional mask products are stored at
\path{artifacts/swe_target_spatial_diagnostic/basin_assignment_grid_wy2021.npz}
and
\path{artifacts/swe_target_spatial_diagnostic/basin_assignment_grid_wy2021.nc}.

\begin{figure}[h]
\centering
\includegraphics[width=0.98\linewidth]{../figures/basin_assignment_test_wy2021.pdf}
\caption{
Diagnostic visualization of the CDEC/DWR basin-based grouping used to assign active April 1 SWE
grid cells to North, Central, and South Sierra regions.
}
\label{fig:basin_assignment_test_wy2021}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.98\linewidth]{../figures/swe_dem_four_panel_diagnostic.pdf}
\caption{
Spatial diagnostic of the Sierra SWE target dataset. The figure compares the April 1 SWE
climatology on the raw Sierra SWE grid, the USGS DEM terrain context over the Sierra Nevada
diagnostic box, the SWE footprint overlaid on the DEM hillshade, and a binary SWE-footprint
diagnostic. This visualization is used to verify that the SWE target grid aligns with the
Sierra Nevada high-elevation region before defining North/Central/South Sierra SWE targets.
}
\label{fig:swe_dem_four_panel_diagnostic}
\end{figure}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.96\linewidth]{../figures/souces_of_long_term_predictability.pdf}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.92\linewidth]{../figures/amv_amo_cobe2_north_atlantic_eofs_modes1to6.pdf}
...
without checking the EOF map and/or correlation with a reference AMV/AMO index.
...
The target variable is April 1 SWE. Because the Sierra Nevada is divided into northern, central, and
...
\qquad
Y_C(t) = \mathrm{SWE}_{C,\mathrm{Apr1}}(t),
\qquad
...
\qquad
X(t,:) \rightarrow Y_C(t),
\qquad
X(t,:) \rightarrow Y_S(t).
\]
The equal-width latitude-band North/Middle/South split used in the first quick ridge baseline
should be treated as provisional only. Before accepting any final regional SWE target definition,
we are now verifying the actual SWE source-grid spatial footprint, the current Sierra mask, and
whether the valid SWE region aligns with the intended Sierra Nevada mountain range.
The same ocean-mode predictor matrix $X$ is used for all three targets, but the regression
coefficients are fitted independently for the northern, central, and southern Sierra targets.
...
\paragraph{Current basin-based LOYO ridge result.}
The first basin-based ocean-mode-only LOYO ridge experiment has now been run with
\path{scripts/run_ocean_mode_only_swe_ridge.py}. The output folder is
\path{artifacts/swe_climate_mode_baseline/ocean_mode_only_ridge/}. The merged predictor table is
\path{artifacts/swe_climate_mode_baseline/ocean_mode_only_ridge/ocean_mode_predictors_wy1985_2021.csv},
and the April 1 SWE target table is
\path{artifacts/swe_climate_mode_baseline/ocean_mode_only_ridge/sierra_apr1_swe_north_central_south_wy1985_2021.csv}.
The LOYO metrics are stored in
\path{artifacts/swe_climate_mode_baseline/ocean_mode_only_ridge/loyo_metrics.json}, and the main
diagnostic figures are
\path{figures/observed_vs_predicted_timeseries.pdf}
and
\path{figures/observed_vs_predicted_scatter.pdf}.
Under strict LOYO validation with the basin-based North/Central/South SWE targets, the current
ocean-mode-only ridge baseline does not yet show useful out-of-sample April 1 SWE skill:
North Sierra has $R^2_{\mathrm{LOYO}}=-1.834$, $\mathrm{RMSE}=0.358\,\mathrm{m}$,
$\mathrm{MAE}=0.307\,\mathrm{m}$, and $r=0.019$; Central Sierra has
$R^2_{\mathrm{LOYO}}=-1.423$, $\mathrm{RMSE}=0.455\,\mathrm{m}$,
$\mathrm{MAE}=0.386\,\mathrm{m}$, and $r=0.119$; South Sierra has
$R^2_{\mathrm{LOYO}}=-1.486$, $\mathrm{RMSE}=0.507\,\mathrm{m}$,
$\mathrm{MAE}=0.427\,\mathrm{m}$, and $r=0.161$. In this run, the inner-LOYO ridge penalty
selection chose $\lambda=10^{-4}$ for all outer held-out years and all three regions. This should
be interpreted conservatively as a first ocean-mode-only baseline, not as evidence that Sierra SWE
is well predicted by the current SST-mode predictor set alone. The time-series and scatter plots
show that the predicted series does not track the observed interannual SWE fluctuations closely,
and many years fall far from the one-to-one line, which is consistent with the negative LOYO
$R^2$ values in all three regions.

\begin{figure}[h]
\centering
\includegraphics[width=0.96\linewidth]{../figures/observed_vs_predicted_timeseries.pdf}
\caption{Observed and LOYO-predicted April 1 SWE for the basin-based North, Central, and South
Sierra targets in the first ocean-mode-only ridge baseline.}
\label{fig:ocean_mode_only_ridge_timeseries}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.96\linewidth]{../figures/observed_vs_predicted_scatter.pdf}
\caption{Observed-versus-predicted April 1 SWE scatter for the basin-based North, Central, and
South Sierra targets in the first ocean-mode-only ridge baseline. The dashed line is the one-to-one
reference line.}
\label{fig:ocean_mode_only_ridge_scatter}
\end{figure}


\subsubsection{Ocean-Mode-Only PLS Predictability Test}
\label{subsubsec:ocean_mode_only_pls_predictability_test}

After the ridge-regression ocean-mode-only test, the next analysis model is partial least squares
regression (PLS). The motivation for adding PLS is that the ocean-mode predictor columns are highly
correlated across months and across related climate modes. Ridge regression shrinks coefficients
but still fits the target directly from the original predictor columns. PLS instead constructs a
small number of supervised latent components from the predictor matrix, so it is a supervised
dimension-reduction method rather than an unsupervised EOF/PCA method.

The scientific question remains the same as in the ridge experiment:
\begin{quote}
Using only ocean-side climate-mode predictors, how much April 1 Sierra SWE variability can be
predicted under strict leave-one-water-year-out cross-validation?
\end{quote}
No December 30 SWE or other same-year observed SWE state is included in this experiment.

The same predictor matrix is used as in the ridge experiment, with $n=37$ water years and $p=91$
predictor columns built from Sep--Mar Pacific SST EOF-PC predictors, Sep--Mar Niño 3.4 indices,
and Sep--Mar North Atlantic AMV/AMO EOF-PC predictors. The three basin-based Sierra SWE targets
are modeled separately:
\[
Y_N(t)=\mathrm{SWE}_{N,\mathrm{Apr1}}(t),\qquad
Y_C(t)=\mathrm{SWE}_{C,\mathrm{Apr1}}(t),\qquad
Y_S(t)=\mathrm{SWE}_{S,\mathrm{Apr1}}(t).
\]
Thus, the PLS experiment fits three independent single-output PLS1 models,
\[
X(t,:) \rightarrow Y_N(t),\qquad
X(t,:) \rightarrow Y_C(t),\qquad
X(t,:) \rightarrow Y_S(t).
\]

\paragraph{PLS model definition.}
For one Sierra target region $R\in\{N,C,S\}$, PLS constructs $K$ latent score vectors from the
training-fold standardized predictor matrix and uses those supervised latent components to predict
the training-fold standardized SWE target. In implementation, this experiment uses a standard
PLS1 algorithm through \texttt{sklearn.cross\_decomposition.PLSRegression} with manual
cross-validation-fold standardization, so the internal scaling option is disabled:
\[
\texttt{PLSRegression(n\_components=K, scale=False)}.
\]
Because the latent components are estimated from the covariance structure between predictors and
target inside each fold, PLS differs from EOF/PCA-style unsupervised reduction and is potentially
better suited to a small, collinear climate-mode predictor set.

\paragraph{Outer leave-one-water-year-out cross-validation.}
Prediction skill is evaluated using strict leave-one-water-year-out cross-validation (LOYO-CV). For
each held-out water year $t_0$, the model is trained on the other 36 years and predicts only the
held-out year. This is done separately for North, Central, and South Sierra, so the final
diagnostic series for each region contains 37 out-of-sample predictions.

\paragraph{Training-fold-only standardization.}
For each outer LOYO split, predictor-column means and standard deviations are computed using only
the 36 outer-training years, and the held-out year is standardized with those training-fold
statistics. The same rule is applied to the target variable within each region. Therefore, the
held-out year is never used to compute predictor scaling, target scaling, latent components,
regression coefficients, or tuning choices.

\paragraph{Inner cross-validation for the number of PLS components.}
The main PLS tuning parameter is the number of latent components $K$. For each outer split, a
small candidate set
\[
K\in\{1,2,3,4,5\}
\]
is tested using an inner leave-one-year-out loop within the 36 outer-training years. Within each
inner split, predictor and target standardization are recomputed using only the 35 inner-training
years. The selected component count is the one that minimizes the mean inner-fold validation mean
squared error, with ties resolved conservatively in favor of the smaller $K$.

\paragraph{Final outer-fold prediction and skill metrics.}
After the optimal $K$ is selected inside one outer LOYO split, the final PLS model is refit on all
36 standardized outer-training years and then used to predict the held-out year. The standardized
prediction is transformed back to physical SWE units using the outer-training target mean and
standard deviation. Repeating this across all 37 water years gives one out-of-sample prediction
series per region. Skill is summarized using
\[
R^2_{\mathrm{LOYO},R},\qquad \mathrm{RMSE}_R,\qquad \mathrm{MAE}_R,\qquad r_R.
\]

Suggested output paths for this ocean-mode-only PLS experiment are:
\begin{quote}
\path{artifacts/swe_climate_mode_baseline/ocean_mode_only_pls/}
\end{quote}
with files such as:
\begin{quote}
\path{artifacts/swe_climate_mode_baseline/ocean_mode_only_pls/loyo_predictions.csv}\\
\path{artifacts/swe_climate_mode_baseline/ocean_mode_only_pls/loyo_metrics.json}\\
\path{artifacts/swe_climate_mode_baseline/ocean_mode_only_pls/selected_components_by_fold.csv}\\
\path{artifacts/swe_climate_mode_baseline/ocean_mode_only_pls/pls_coefficients_by_fold.csv}\\
\path{artifacts/swe_climate_mode_baseline/ocean_mode_only_pls/pls_loadings_weights_by_fold.nc}\\
\path{artifacts/swe_climate_mode_baseline/ocean_mode_only_pls/observed_vs_predicted_timeseries.png}\\
\path{artifacts/swe_climate_mode_baseline/ocean_mode_only_pls/run.log}
\end{quote}

\paragraph{Current basin-based LOYO PLS result.}
The first basin-based ocean-mode-only LOYO PLS experiment has now been run with
\path{scripts/run_ocean_mode_only_swe_pls.py}. The output folder is
\path{artifacts/swe_climate_mode_baseline/ocean_mode_only_pls/}. The merged predictor table is
\path{artifacts/swe_climate_mode_baseline/ocean_mode_only_pls/ocean_mode_predictors_wy1985_2021.csv},
and the April 1 SWE target table is
\path{artifacts/swe_climate_mode_baseline/ocean_mode_only_pls/sierra_apr1_swe_north_central_south_wy1985_2021.csv}.
The LOYO metrics are stored in
\path{artifacts/swe_climate_mode_baseline/ocean_mode_only_pls/loyo_metrics.json}; the foldwise
selected component counts are stored in
\path{artifacts/swe_climate_mode_baseline/ocean_mode_only_pls/selected_components_by_fold.csv}; the
foldwise coefficients are stored in
\path{artifacts/swe_climate_mode_baseline/ocean_mode_only_pls/pls_coefficients_by_fold.csv}; and
the foldwise PLS weights and loadings are stored in
\path{artifacts/swe_climate_mode_baseline/ocean_mode_only_pls/pls_loadings_weights_by_fold.nc}. The
main diagnostic figures are \path{figures/ocean_mode_only_pls_observed_vs_predicted_timeseries.pdf},
\path{figures/ocean_mode_only_pls_observed_vs_predicted_scatter.pdf}, and
\path{figures/ocean_mode_only_pls_selected_components_by_fold.pdf}.

Under strict LOYO validation with the basin-based North/Central/South SWE targets, the current
ocean-mode-only PLS baseline improves on the ridge baseline but still does not yet show useful
standalone out-of-sample April 1 SWE skill. North Sierra has
$R^2_{\mathrm{LOYO}}=-0.371$, $\mathrm{RMSE}=0.249\,\mathrm{m}$,
$\mathrm{MAE}=0.199\,\mathrm{m}$, and $r=-0.161$; Central Sierra has
$R^2_{\mathrm{LOYO}}=-0.487$, $\mathrm{RMSE}=0.357\,\mathrm{m}$,
$\mathrm{MAE}=0.283\,\mathrm{m}$, and $r=-0.241$; South Sierra has
$R^2_{\mathrm{LOYO}}=-0.434$, $\mathrm{RMSE}=0.385\,\mathrm{m}$,
$\mathrm{MAE}=0.315\,\mathrm{m}$, and $r=-0.025$. The inner-LOYO component selection favored a
small latent space: North selected $K=1$ in 31 of 37 folds and $K=2$ in 6 folds; Central selected
$K=1$ in 21 folds and $K=2$ in 16 folds; South selected $K=1$ in 22 folds, $K=2$ in 14 folds, and
$K=4$ in 1 fold. The time-series and scatter plots show that the PLS predictions are somewhat less
dispersed than the ridge predictions, but they still fail to track the observed interannual SWE
fluctuations closely and remain close to the climatological mean in many years. This should
therefore be interpreted conservatively as an improved but still weak ocean-mode-only baseline, not
as evidence that the current SST-mode predictor family alone provides robust April 1 Sierra SWE
predictability.

\begin{figure}[h]
\centering
\includegraphics[width=0.96\linewidth]{../figures/ocean_mode_only_pls_observed_vs_predicted_timeseries.pdf}
\caption{Observed and LOYO-predicted April 1 SWE for the basin-based North, Central, and South
Sierra targets in the ocean-mode-only PLS baseline.}
\label{fig:ocean_mode_only_pls_timeseries}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.96\linewidth]{../figures/ocean_mode_only_pls_observed_vs_predicted_scatter.pdf}
\caption{Observed-versus-predicted April 1 SWE scatter for the basin-based North, Central, and
South Sierra targets in the ocean-mode-only PLS baseline. The dashed line is the one-to-one
reference line.}
\label{fig:ocean_mode_only_pls_scatter}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.80\linewidth]{../figures/ocean_mode_only_pls_selected_components_by_fold.pdf}
\caption{Selected number of latent PLS components for each outer LOYO fold in the ocean-mode-only
PLS baseline. Most folds select only one or two components, indicating that the tuned model favors
a very low-dimensional supervised representation of the predictor matrix.}
\label{fig:ocean_mode_only_pls_components}
\end{figure}

\subsection{Mixing LOD-Selected SSTs with Large-Scale SST Modes}

We use the first two full-37-year LOD-selected SST columns as reference predictors, denoted
\(Z_1\) and \(Z_2\). These are the full-sample selected mode-1 and mode-2 SST time series. We then
tested whether their apparent skill is stable under stricter settings and whether large-scale SST
modes add missing information.


\begin{itemize}
\item \textbf{Fixed \(Z_1+Z_2\) oracle test.}
Using the full-37 selected \(Z_1\) and \(Z_2\) as fixed predictors gives moderate LOYO skill:
\(r\approx 0.55\), \(R^2\approx 0.30\), RMSE \(\approx 0.0265\,\mathrm{m}\), and sign accuracy
\(\approx 0.78\). This shows that the two full-sample LOD-selected SST columns contain useful
SWE-related information, but this is an oracle-style test because the selected locations are
known from all 37 years. The result is shown in \cref{fig:exact_z1_z2_residual_timeline}.

\begin{figure}[h]
\centering
\includegraphics[width=0.96\linewidth]{../figures/exact_Z1_Z2_residual_timeline.pdf}
\caption{Observed and predicted April 1 SWE anomalies for the fixed full-sample \(Z_1+Z_2\)
oracle-style LOYO test.}
\label{fig:exact_z1_z2_residual_timeline}
\end{figure}

\item \textbf{Nested \(M_1+M_2\) LOD test.}
In the strict nested LOYO test, LOD re-selects modes inside each 36-year training fold. This
does not retain the fixed \(Z_1+Z_2\) skill. The largest-error years also differ from the fixed
oracle model: nested \(M_1+M_2\) fails most strongly in 2011, 2015, 2006, 1993, and 2019,
whereas fixed \(Z_1+Z_2\) fails most strongly in 1993, 2012, 2005, 2021, and 2006. Only 1993
and 2006 overlap. This indicates that nested LOD failure is not just the same residual problem;
feature-selection instability is also important. The result is shown in
\cref{fig:nested_lod_m1_m2_vs_oracle_timeseries}.

\begin{figure}[h]
\centering
\includegraphics[width=0.96\linewidth]{../figures/nested_lod_m1_m2_vs_oracle_timeseries.pdf}
\caption{Observed SWE anomalies compared against the strict nested \(M_1+M_2\) LOD LOYO
prediction and the fixed full-sample \(Z_1+Z_2\) oracle-style prediction.}
\label{fig:nested_lod_m1_m2_vs_oracle_timeseries}
\end{figure}

\item \textbf{Patch-average test around \(Z_1,Z_2\).}
We replaced the exact grid-cell predictors with \(5^\circ\), \(10^\circ\), and \(15^\circ\)
patch averages around the \(Z_1,Z_2\) locations. Patch averaging did not improve prediction.
Skill decreased as patch size increased. Therefore, the useful \(Z_1,Z_2\) signal is not a
simple broad regional box-average SST signal. The result is shown in
\cref{fig:full37_patch_observed_vs_predicted}.

\begin{figure}[h]
\centering
\includegraphics[width=0.96\linewidth]{../figures/full37_patch_observed_vs_predicted.pdf}
\caption{Observed-versus-predicted April 1 SWE anomalies for the patch-average test around the
fixed \(Z_1,Z_2\) predictor locations.}
\label{fig:full37_patch_observed_vs_predicted}
\end{figure}

\item \textbf{\(Z_1+Z_2\) residual analysis.}
The residual
\[
r(y)=SWE_{\mathrm{obs}}(y)-\widehat{SWE}_{Z_1+Z_2}(y)
\]
is structured, not random. The fixed \(Z_1+Z_2\) model underpredicts several wet years and
overpredicts several dry years. The main residual years are 1993, 2005, 2006, 2012, and 2021.
This suggests that \(Z_1+Z_2\) captures part of the SST-related SWE signal but misses some
extreme wet/dry behavior.
\item \textbf{Residual screening against large-scale SST modes.}
We screened the \(Z_1+Z_2\) residuals against Pacific PC1--PC6, Ni\~no 3.4, and AMV/AMO
PC1--PC6 from September through March. AMV/AMO showed the strongest residual association,
Pacific PCs were weaker, and Ni\~no 3.4 was weakest. These were screening correlations only,
not predictive tests.

\item \textbf{Adding AMV/AMO PCs to \(Z_1+Z_2\).}
We added AMV/AMO PCs from September through March to the \(Z_1+Z_2\) ridge model. Although the
AMV/AMO-only model has useful standalone skill, the combined
\(Z_1+Z_2+\) AMV/AMO model performs worse than either component model. Thus, the two predictor
blocks are not simply additive under the current ridge LOYO setup. This may reflect collinearity,
foldwise coefficient instability, or the small-sample penalty from combining two predictor
families. The time-series comparison is shown in \cref{fig:focused_z1z2_amv_timeseries}, and the
observed-versus-predicted scatter comparison is shown in \cref{fig:focused_z1z2_amv_scatter}.

\begin{figure}[h]
\centering
\includegraphics[width=0.96\linewidth]{../figures/focused_z1z2_amv_timeseries.pdf}
\caption{Strict LOYO time-series comparison for the fixed \(Z_1+Z_2\) model, the AMV/AMO
PC1--PC6-only model, and the combined \(Z_1+Z_2+\) AMV/AMO PC1--PC6 model.}
\label{fig:focused_z1z2_amv_timeseries}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.92\linewidth]{../figures/focused_z1z2_amv_scatter.pdf}
\caption{Strict LOYO observed-versus-predicted scatter comparison for the fixed \(Z_1+Z_2\)
model, the AMV/AMO PC1--PC6-only model, and the combined \(Z_1+Z_2+\) AMV/AMO PC1--PC6 model.}
\label{fig:focused_z1z2_amv_scatter}
\end{figure}

\item \textbf{Adding Pacific PCs and Ni\~no 3.4 to ridge LOYO.}
We also tested \(Z_1+Z_2+\) Pacific PC1--PC6, \(Z_1+Z_2+\) Ni\~no 3.4, and
\(Z_1+Z_2+\) Pacific PC1--PC6 \(+\) Ni\~no 3.4. None improved over the \(Z_1+Z_2\)
ridge baseline. Pacific PCs degraded strongly; Ni\~no 3.4 was less damaging but still did not
improve prediction.

\item \textbf{Oracle-column support years.}
The full-sample LOD columns are not supported uniformly by all years. Leave-one-year influence
tests show that the clearest support years for keeping \(Z_1\) competitive are
\(2012, 2005, 2006, 2017,\) and \(1994\), while the clearest support years for \(Z_2\) are
\(2017, 1994, 2011, 2019,\) and \(1992\). Their union,
\(1992, 1994, 2005, 2006, 2011, 2012, 2017,\) and \(2019\),
overlaps substantially with the worst nested \(M_1+M_2\) prediction years: it captures 3 of the
top 5 worst years and 6 of the top 10 worst years. This means several bad nested-LOD years are not
only hard prediction years; they are also years whose removal changes the selected SST columns.
This overlap is illustrated again in \cref{fig:nested_lod_m1_m2_vs_oracle_timeseries_support}.

\begin{figure}[h]
\centering
\includegraphics[width=0.96\linewidth]{../figures/nested_lod_m1_m2_vs_oracle_timeseries.pdf}
\caption{Observed SWE anomalies compared against the strict nested \(M_1+M_2\) LOD LOYO
prediction and the fixed full-sample \(Z_1+Z_2\) oracle-style prediction, shown here to highlight
the overlap between the worst nested years and the oracle-column support years.}
\label{fig:nested_lod_m1_m2_vs_oracle_timeseries_support}
\end{figure}

\item \textbf{Near-miss versus wrong-column LOD failures.}
The exact-match criterion is too strict because some non-exact nested selections remain in the
same month and near the oracle location. Mode 1 has 17 exact matches and 9 near misses, so many of
its non-exact selections are still close to \(Z_1\). Mode 2 is less stable: it has 18 exact matches
but 15 true wrong-column selections. The worst nested \(M_1+M_2\) years are concentrated in true
wrong-column cases. In particular, 2011, 2015, 2006, 1993, and 2019 all involve a true wrong or
non-acceptable selection. Therefore, nested LOD does not fail merely because it misses the exact
grid cell; it fails mainly when the selected column jumps to a different month and far-away region.


\item \textbf{Reduced AMV/AMO core versus full AMV/AMO block.}
The AMV/AMO-only ridge experiments show two layers of signal. A stable four-feature starting core,
\[
\{\mathrm{AMV\_PC4\_Sep},\ \mathrm{AMV\_PC5\_Feb},\ \mathrm{AMV\_PC2\_Feb},\
\mathrm{AMV\_PC4\_Nov}\},
\]
captures a compact directional component. We then added remaining AMV/AMO PC-month predictors
one at a time and stopped when two consecutive additions failed to give a meaningful LOYO gain.
The only useful additional predictor was \(\mathrm{AMV\_PC5\_Mar}\), giving the reduced five-feature
AMV/AMO core
\[
\{\mathrm{AMV\_PC4\_Sep},\ \mathrm{AMV\_PC5\_Feb},\ \mathrm{AMV\_PC2\_Feb},\
\mathrm{AMV\_PC4\_Nov},\ \mathrm{AMV\_PC5\_Mar}\}.
\]
This K5 reduced core improves over the K4 starting core, reaching \(R^2=0.300\), RMSE
\(=0.02653\,\mathrm{m}\), and sign accuracy \(=0.757\). The full AMV/AMO PC1--PC6
September--March ridge model still gives slightly better amplitude metrics
(\(R^2=0.339\), RMSE \(=0.02579\,\mathrm{m}\)), but lower sign accuracy (\(0.649\)).
Thus, most of the stable low-dimensional AMV/AMO signal is captured by the five-feature reduced
core, while the remaining amplitude gain in the full model is distributed across many weaker
PC-month predictors. The observed-versus-predicted time series is shown in
\cref{fig:amv_amo_reduced_core_observed_vs_predicted}, and the corresponding scatter comparison is
shown in \cref{fig:amv_amo_reduced_core_scatter}.
\begin{figure}[h]
\centering
\includegraphics[width=0.96\linewidth]{../figures/amv_amo_reduced_core_observed_vs_predicted.pdf}
\caption{Observed April 1 Sierra SWE anomalies compared against the K4 starting AMV/AMO core, the
K5 reduced AMV/AMO core, and the full 42-feature AMV/AMO ridge model under strict LOYO evaluation.}
\label{fig:amv_amo_reduced_core_observed_vs_predicted}
\end{figure}
\begin{figure}[h]
\centering
\includegraphics[width=0.92\linewidth]{../figures/amv_amo_reduced_core_scatter.pdf}
\caption{Observed-versus-predicted scatter comparison for the K4 starting AMV/AMO core, the K5
reduced AMV/AMO core, and the full 42-feature AMV/AMO ridge model.}
\label{fig:amv_amo_reduced_core_scatter}
\end{figure}
\end{itemize}

As a final closure test for the SST-only linear branch, we compared the fixed \(Z_1+Z_2\) LOD
reference model against full large-scale SST predictor blocks and their combinations using the
same strict LOYO ridge protocol.

\input{../artifacts/cobe2_sierra_swe_lod_setup/final_sst_only_linear_closure_table/final_sst_only_closure_table.tex}

The fixed \(Z_1+Z_2\) model remains the best low-dimensional fixed-LOD reference and ties for the
best sign accuracy, but AMV/AMO PC1--PC6 alone is strongest by \(r\), \(R^2\), RMSE, and MAE.
Adding Pacific PCs, Ni\~no 3.4, or AMV/AMO PCs to \(Z_1+Z_2\) does not improve the fixed
\(Z_1+Z_2\) baseline.
