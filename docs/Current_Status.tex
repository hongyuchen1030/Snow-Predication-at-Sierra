...
feature-selection instability is also important.
...
simple broad regional box-average SST signal.
...
\item \textbf{Adding AMV/AMO PCs to ridge LOYO.}
We added AMV/AMO PC2--PC4 from September through March to the \(Z_1+Z_2\) ridge model. This
did not improve strict LOYO prediction. The combined model performed worse than the
\(Z_1+Z_2\) ridge baseline.
...
improve prediction.
\item \textbf{Current conclusion.}
The fixed full-sample \(Z_1+Z_2\) pair is the best SST-only linear predictor found so far.
However, its skill depends on knowing the full-sample selected locations. Nested LOD does not
reliably recover the same skill, patch averaging does not stabilize it, and adding standard
large-scale SST modes does not improve strict LOYO prediction. Thus, the current linear
SST-only branch has reached a natural stopping point.
